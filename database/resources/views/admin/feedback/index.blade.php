                ...
                <table class="table table-bordered table-hover text-center">
                    ...
                    @foreach($data as $item)
                        <tr>
                            ...
                            <td>
                                @if ($item->is_read == 1)
                                    <a href="{{ route('feedback.check', ['id' => $item->getKey(), 'status' => 0]) }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo _trans('words.are_you_sure_unread') ?>')">{{__('words.unread')}}</a>
                                @elseif($item->is_read == 0)
                                    <a href="{{ route('feedback.check', ['id' => $item->getKey(), 'status' => 1]) }}" class="btn btn-success btn-xs" onclick="return confirm('<?php echo _trans('words.are_you_sure_read') ?>')">{{__('words.Have_read')}}</a>
                                @endif
