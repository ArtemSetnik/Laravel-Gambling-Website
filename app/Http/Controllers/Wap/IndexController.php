...
class IndexController extends WebBaseController
{
    ...
    public function hotgame($type)
    {
        $member = auth('member')->user();
        if($member == null){
            return view('wap.hotgame_nosign',compact('type'));
        }else{
            return view('wap.hotgame',compact('type'),['member'=>$member]);
        }
...
//    }
    ...
    public function post_drawing(Request $request)
    {
        ...
        return responseSuccess('',__(words.Withdraw_Successful), route('wap.drawing_record'));
    }
