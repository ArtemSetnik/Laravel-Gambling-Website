                        ...
                        @foreach(config('admin_menu') as $item)
                            <tr class="row">
                                <td class="col-md-1 pull-left">
                                    <strong style="color: red">{{ __($item['name']) }}</strong>
                                </td>
                            </tr>
                            ...
                            <tr class="row">
                                ...
                                @foreach($item['submenus'] as $v)
                                    <td class="col-md-1 pull-left">
                                        <label>
                                            <input type="checkbox" name="routers[]" value="{{ $v['router'] }}" @if(in_array($v['router'], $check_routers)) checked @endif @if($v['is_hide'] == 0) disabled @endif>{{ __($v['title']) }}
                                        </label>
                                    </td>
                                @endforeach
                            </tr>
                            <tr class="row">
                                ...
                                @foreach($item['submenus'] as $v)
                                    @foreach($v['actions'] as $val)
                                        <td class="col-md-1 pull-left">
                                            <label>
                                                <input type="checkbox" name="routers[]" value="{{ $val['router'] }}" @if(in_array($val['router'], $check_routers)) checked @endif>{{ __($val['name']) }}
                                            </label>
                                        </td>
                                    @endforeach
                                @endforeach
