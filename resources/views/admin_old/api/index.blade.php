                 ...
                 <table class="table table-bordered table-hover text-center">
                     ...
                     @foreach($data as $item)
                         ...
                         <tr>
                             ...
                             <td>
                                 @if ($item->on_line == 0)
                                     <a href="{{ route('apple.check', ['id' => $item->getKey(), 'status' => 1]) }}" class="btn btn-danger btn-xs" onclick="return confirm('确定下线吗？')">{{__('words.Offline')}}</a>
                                 @elseif($item->on_line == 1)
                                     <a href="{{ route('apple.check', ['id' => $item->getKey(), 'status' => 0]) }}" class="btn btn-success btn-xs" onclick="return confirm('确定上线吗？')">{{__('words.online')}}</a>
                                 @endif
